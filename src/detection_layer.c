...
void get_detection_boxes(layer l, int w, int h, float thresh, float **probs, box *boxes, int only_objectness)
{
    ...
    for (int i = 0; i < l.side*l.side; ++i){
        const int row = i / l.side;
        const int col = i % l.side;
        for(int n = 0; n < l.n; ++n){
            const int index = i*l.n + n;
            const int p_index = l.side*l.side*l.classes + i*l.n + n;
            const float scale = predictions[p_index];
            const int box_index = l.side*l.side*(l.classes + l.n) + (i*l.n + n)*4;
            ...
            for(int j = 0; j < l.classes; ++j){
                const int class_index = i*l.classes;
